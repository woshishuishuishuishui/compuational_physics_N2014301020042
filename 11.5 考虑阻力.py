...
import pylab as pl
class wave:
    def __init__(self,L=0.65,c=320,FT=149,dx=0.00065,T=0.0203125,b=1/5,a=1/2):
        self.L=L
        self.c=c
        self.FT=FT
        self.dx=dx
    ...
    def propagate(self):
        ...
        while self.t[-1]<self.T:
            ...
            self.y3.append(0)
            for i in range(int(self.L/self.dx)-1):
                self.y3.append(-self.y1[i+1]+self.y2[i+2]+self.y2[i]-self.a*self.dx/self.c*(self.y2[i+2]-self.y2[i+1]))
            self.y3.append(0)    
